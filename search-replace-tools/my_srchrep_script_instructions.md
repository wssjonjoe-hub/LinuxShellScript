Save this as `srchrep.sh` in the directory where you want to run it:
...
Test the replacement first with `--dry-run`:

```bash
./srchrep.sh 'http://domain.com' 'https://domain.com' --dry-run
```

If the output looks correct, apply the changes:

```bash
./srchrep.sh 'http://domain.com' 'https://domain.com'
```

The script:

- Searches recursively from the current directory.
- Skips `.git`, `.svn`, `.hg`, and `node_modules`.
- Skips binary files.
- Shows unified diffs during `--dry-run`.
- Only changes files containing the search text.
